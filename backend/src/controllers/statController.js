...
exports.getDashboardStats = async (req, res) => {
    try {
        ...
        const [collabEnterprisesRows] = await pool.query(`SELECT COUNT(*) as count FROM enterprises WHERE status = 'Đang hợp tác' ${facultyFilter}`, params.length > 0 ? [params[1]] : []);
        ...
        const [studentsRows] = await pool.query(`
            ...
            WHERE 1=1 ${facultyFilter}`, params.length > 0 ? [params[3]] : []);
